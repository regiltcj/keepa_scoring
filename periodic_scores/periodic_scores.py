...
def compute_periodic_scores(brands):
    ...
    logger.info("Continuing product_concentration score evaluation")
    ...
    out_of_stocks = pd.read_csv("./datasets/out_of_stock.csv")
    out_of_stocks = get_out_of_pct_score(out_of_stocks)

    brands = pd.merge(brands, out_of_stocks, on="brand", how="left", validate="one_to_one")
    del out_of_stocks
